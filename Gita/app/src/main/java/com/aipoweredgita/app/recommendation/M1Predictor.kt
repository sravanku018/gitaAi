...
class M1Predictor(
    ...
    suspend fun predictNext(): NextSuggestion {
        val dateFormatter = DateTimeFormatter.ISO_LOCAL_DATE

        // Gather last 7 days activity
        val last7 = mutableListOf<com.aipoweredgita.app.database.DailyActivity?>()
        repeat(7) {
            val d = LocalDate.now().minusDays(it.toLong()).format(dateFormatter)
            val row = try { dailyActivityDao.getByDate(d) } catch (_: Exception) { null }
            last7.add(row)
        }
        ...
        val streak = try {
            userStatsDao.getUserStatsOnce()?.currentStreak ?: 0
        } catch (_: Exception) { 0 }
