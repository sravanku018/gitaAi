...
abstract class GitaDatabase : RoomDatabase() {
    ...
    companion object {
        ...
        private val MIGRATION_42_43 = object : Migration(42, 43) {
            override fun migrate(database: SupportSQLiteDatabase) {
                // Clear old quiz questions that have no language tag (modelVersion = '')
                // These were imported with language='all' before v2.0.1 and mixed EN+TE.
                // They will be re-imported correctly (English only) on next app start.
                database.execSQL(
                    "DELETE FROM quiz_question_bank WHERE modelVersion = '' OR modelVersion IS NULL"
                )
            }
        }
        ...
        private val MIGRATION_43_44 = object : Migration(43, 44) {
            override fun migrate(database: SupportSQLiteDatabase) {
                // Delete old dataset questions so clean 100% pure Telugu options are auto-imported
                database.execSQL("DELETE FROM quiz_question_bank WHERE generationMethod = 'dataset_import'")
            }
        }

        private val MIGRATION_44_45 = object : Migration(44, 45) {
            override fun migrate(database: SupportSQLiteDatabase) {
                // Add dedicated language column and index for fast, isolated queries
                database.execSQL("ALTER TABLE quiz_question_bank ADD COLUMN language TEXT NOT NULL DEFAULT 'english'")
                database.execSQL("CREATE INDEX IF NOT EXISTS index_quiz_question_bank_language ON quiz_question_bank(language)")
                // Update language column from modelVersion if present
                database.execSQL("UPDATE quiz_question_bank SET language = LOWER(modelVersion) WHERE modelVersion IS NOT NULL AND modelVersion != ''")
            }
        }

        private val MIGRATION_45_46 = object : Migration(45, 46) {
            override fun migrate(database: SupportSQLiteDatabase) {
                database.execSQL("CREATE INDEX IF NOT EXISTS index_quiz_question_bank_language_isActive_difficulty_lastAskedAt ON quiz_question_bank(language, isActive, difficulty, lastAskedAt)")
            }
        }

        private val MIGRATION_46_47 = object : Migration(46, 47) {
            override fun migrate(database: SupportSQLiteDatabase) {
                database.execSQL("ALTER TABLE quiz_attempts ADD COLUMN attemptId TEXT NOT NULL DEFAULT ''")
                database.execSQL("ALTER TABLE quiz_attempts ADD COLUMN syncStatus TEXT NOT NULL DEFAULT 'PENDING'")
                database.execSQL("UPDATE quiz_attempts SET attemptId = 'legacy_' || id WHERE attemptId = ''")
                database.execSQL("CREATE UNIQUE INDEX IF NOT EXISTS index_quiz_attempts_attemptId ON quiz_attempts(attemptId)")
            }
        }

        private val MIGRATION_47_48 = object : Migration(47, 48) {
            override fun migrate(database: SupportSQLiteDatabase) {
                database.execSQL("ALTER TABLE quiz_attempts ADD COLUMN language TEXT NOT NULL DEFAULT 'en'")
            }
        }

        private val MIGRATION_48_49 = object : Migration(48, 49) {
            override fun migrate(database: SupportSQLiteDatabase) {
                database.execSQL("ALTER TABLE verse_notes ADD COLUMN colorHex TEXT NOT NULL DEFAULT ''")
            }
        }

        private val MIGRATION_49_50 = object : Migration(49, 50) {
            override fun migrate(database: SupportSQLiteDatabase) {
                // Version 50 bump migration
            }
        }
