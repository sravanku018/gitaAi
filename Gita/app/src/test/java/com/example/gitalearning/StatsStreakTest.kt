...
class StatsStreakTest {
    ...
    @Test
    fun testStreakWithIsoTimestamp() = runBlocking {
        val dao = FakeUserStatsDao()
        val pendingDao = FakePendingSyncEventDao()
        val repo = StatsRepository(
            userStatsDao = dao,
        ...
        // Set lastActiveDate to a full ISO timestamp from yesterday
        val yesterday = java.time.LocalDate.now().minusDays(1)
        val isoTimestamp = "${yesterday}T12:34:56.789Z"
        
        // Start with a streak of 5
        dao.stats = UserStats(
            userId = "test_user_id",
            lastActiveDate = isoTimestamp,
            currentStreak = 5,
            longestStreak = 5,
            daysActive = 10
        )
        ...
        // The streak should increment to 6, because the date part of the ISO string matches yesterday
        assertEquals("Streak should be 6", 6, s.currentStreak)
        assertEquals("Longest streak should be 6", 6, s.longestStreak)
        assertEquals("Days active should be 11", 11, s.daysActive)
    }
}
