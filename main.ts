...
/** SQLite/Turso-compatible execute adapter for PostgreSQL */
async function pgExecute(
  sqlOrOpts: string | { sql: string; args?: any[] },
  maybeArgs?: any[],
) {
  let sql: string;
  let args: any[] = [];
  ...
  let paramIndex = 1;
  let pgSql = sql
    .replace(/datetime\('now',\s*'\+30 days'\)/gi, "(NOW() + INTERVAL '30 days')::text")
    .replace(/datetime\("now",\s*"\+30 days"\)/gi, "(NOW() + INTERVAL '30 days')::text")
    .replace(/datetime\('now'\)/gi, "CURRENT_TIMESTAMP")
    .replace(/datetime\("now"\)/gi, "CURRENT_TIMESTAMP")
    .replace(/INSERT OR IGNORE INTO/gi, "INSERT INTO")
    .replace(/INSERT OR REPLACE INTO/gi, "INSERT INTO")
    // SQLite AUTOINCREMENT → ignore (tables already exist on VPS)
    .replace(/INTEGER PRIMARY KEY AUTOINCREMENT/gi, "SERIAL PRIMARY KEY")
    // text expires_at vs timestamp comparisons
    .replace(
      /expires_at\s*>\s*CURRENT_TIMESTAMP/gi,
      "expires_at::timestamptz > NOW()",
    )
    .replace(
      /expires_at\s*>\s*NOW\(\)/gi,
      "expires_at::timestamptz > NOW()",
    );

  // Multi-arg MIN/MAX used for coin caps → LEAST/GREATEST; keep single-arg aggregates
  pgSql = pgSql.replace(/\bMIN\(([^()]*?,[^()]*?)\)/gi, "LEAST($1)");
  pgSql = pgSql.replace(/\bMAX\(([^()]*?,[^()]*?)\)/gi, "GREATEST($1)");
  ...
  try {
    ...
    if (hadIgnore && err?.code === "23505") return { rows: [], rowCount: 0 };
...
async function requireAuth(c: any, next: any) {
  const token = c.req.header("Authorization")?.replace(/^Bearer\s+/i, "") || c.req.header("authorization")?.replace(/^Bearer\s+/i, "");
  ...
  if (session.rows.length) {
    userId = session.rows[0].user_id as string;
  ...
  // Ensure users and user_stats rows exist so coin updates and dashboard queries never fail
  if (userId.startsWith("guest_")) {
    await db.execute({
      sql: `INSERT OR IGNORE INTO users (user_id, password_hash, name, email, is_guest, created_at) VALUES (?, '', 'Guest', ?, 1, datetime('now'))`,
      args: [userId, `${userId}@gita.com`]
    }).catch(() => {});
  }
  await db.execute({
    sql: `INSERT OR IGNORE INTO user_stats (user_id, krishna_coins, days_active, yoga_level, last_activity_date) VALUES (?, 50, 1, 1, ?)`,
...
// ─── ROOT ────────────────────────────────────────────────────
app.get("/", (c) => c.json({
  status: "Gita App API v2.14.0 running on PostgreSQL (VPS) ✅",
  version: "2.14.0",
  endpoints: [
    "POST /auth/register",
    "POST /auth/login",
...
app.post("/guest/create", async (c) => {
  try {
    ...
    // 1. Insert into users table with password_hash = '' and created_at
    await db.execute({
      sql: `INSERT INTO users (user_id, password_hash, name, email, is_guest, created_at) VALUES (?, '', 'Guest', ?, 1, datetime('now'))`,
      args: [guest_id, guest_email]
    });

    // 2. Insert user stats
    await db.execute({
      sql: `INSERT INTO user_stats (user_id, krishna_coins, days_active, yoga_level, last_activity_date) VALUES (?, 50, 1, 1, ?)`,
      args: [guest_id, todayStr]
    }).catch(e => console.error("Error creating guest user_stats:", e?.message));

    // 3. Insert streak
    await db.execute({
      sql: `INSERT OR IGNORE INTO checkin_streaks (user_id, current_day, current_week, share_day, share_week) VALUES (?, 0, 1, 0, 1)`,
      args: [guest_id]
    }).catch(e => console.error("Error creating guest streak:", e?.message));

    // 4. Insert welcome bonus transaction
    await db.execute({
      sql: `INSERT INTO coin_transactions (user_id, amount, type, source, description, created_at) VALUES (?, 50, 'EARN', 'signup', 'Guest welcome bonus', datetime('now'))`,
      args: [guest_id]
    }).catch(e => console.error("Error creating guest transaction:", e?.message));
    ...
    console.error("CRITICAL GUEST CREATE ERROR:", err?.message || err);
    return c.json({ error: "Failed to create guest user", details: err?.message || String(err) }, 500);
  }
});
...
// ─── STREAK HELPER ─────────────────────────────────────────────
/**
 * Activity streak reconciliation for /users/stats/sync.
 *
 * Rules (server is source of truth for the NUMBER; client date drives calendar):
 * - Same day: keep server streak (do NOT re-increment; ignore inflated client counts)
 * - +1 day (or +2 grace for TZ): server_streak + 1 only (no max(client)+1 double-count)
 * - Gap > 2 days: RESET to 1
 * - Client behind server: keep server
 *
 * Previous bugs: max(server,client)+1 on consecutive days inflated streaks;
 * gaps did not reset so stale high streaks never died.
 */
function calculateStreak(
  serverLastActivityDate: string | null,
  serverCurrentStreak: number,
  clientLastActivityDate: string,
  _clientCurrentStreak: number,
): { current_streak: number; last_activity_date: string } {
  const serverDate = (serverLastActivityDate || "").substring(0, 10);
  const clientDate = (clientLastActivityDate || "").substring(0, 10);
  const safeServerStreak = Math.max(0, serverCurrentStreak || 0);
  const todayIso = new Date().toISOString().split("T")[0];

  if (!clientDate && !serverDate) {
    return {
      current_streak: Math.max(1, safeServerStreak),
  ...
  if (!serverDate) {
    // No server record of last activity (legacy/pre-tracking user, or corrupted row).
    // Can't verify any continuity, so never trust the client's claimed streak here —
    // that's the same client-supplied-counter drift bug fixed elsewhere. Start fresh.
    return {
      current_streak: 1,
  ...
  if (dayDiff < 0) {
    // Client clock / cache behind — keep authoritative server
    return { current_streak: safeServerStreak, last_activity_date: serverDate };
  }
  if (dayDiff === 0) {
    // Same calendar day: never bump again; do not adopt inflated client values
    return { current_streak: safeServerStreak, last_activity_date: clientDate };
  }
  if (dayDiff === 1 || dayDiff === 2) {
    // Consecutive day, or 1-day timezone grace (matches app updateStreak grace)
    // Increment SERVER streak only — never max(client)+1 (that double-counted).
    const streak = safeServerStreak <= 0 ? 1 : safeServerStreak + 1;
    return { current_streak: streak, last_activity_date: clientDate };
  }
  // Missed more than grace window — reset
  return { current_streak: 1, last_activity_date: clientDate };
}
...
app.post("/users/stats/sync", requireAuth, async (c) => {
  ...
  // 1. Ensure user_stats row exists first
  await db.execute({
    sql: `INSERT OR IGNORE INTO user_stats (user_id, krishna_coins, days_active, yoga_level, last_activity_date) VALUES (?, 50, 1, 1, ?)`,
    args: [user_id, new Date().toISOString().split("T")[0]]
  }).catch(() => {});
  ...
  const serverStats = await db.execute({
    sql: `SELECT krishna_coins, yoga_level, days_active, current_streak, longest_streak, total_quizzes_taken, total_questions_answered,
  ...
  // ── Monotonically increasing / MAX stats — NEVER PULL BACK ──
  const newCoins = Math.max((server.krishna_coins as number) || 0, krishna_coins || 0);
  const newYogaLevel = Math.max((server.yoga_level as number) || 1, yoga_level || 1);
  ...
  // ── Return complete authoritative server state to client ──
...
app.get("/coins/balance", requireAuth, async (c) => {
  ...
  // current_streak is only recomputed inside POST /users/stats/sync.
  // If the client reads balance without syncing first, a stale streak
  // from before an inactivity gap was returned as-is. Recompute the
  // gap here too, and persist it so it doesn't drift from stats/sync's logic.
  const lastDate = String(row.last_activity_date || "").substring(0, 10);
  const todayIso = new Date().toISOString().split("T")[0];
  if (row.current_streak > 0) {
    let shouldReset = false;
    if (!lastDate) {
      // Legacy user, no recorded last-activity date — can't verify continuity
      shouldReset = true;
    } else {
      const sTime = new Date(lastDate + "T00:00:00Z").getTime();
      const tTime = new Date(todayIso + "T00:00:00Z").getTime();
      if (!isNaN(sTime) && !isNaN(tTime)) {
        const dayDiff = Math.floor((tTime - sTime) / (1000 * 60 * 60 * 24));
        if (dayDiff > 2) shouldReset = true;
      }
    }
    if (shouldReset) {
      row.current_streak = 0;
      await db.execute({
        sql: `UPDATE user_stats SET current_streak = 0, updated_at = datetime('now') WHERE user_id = ?`,
        args: [user_id],
      }).catch(() => {});
    }
  }
...
app.post("/coins/spend", requireAuth, async (c) => {
  const user_id = c.get("userId" as any) as string;
  const { amount, source, description, question, idempotency_key, client_date, country_code } = await c.req.json().catch(() => ({}));
  if (country_code) await updateUserCountry(user_id, country_code);
  const dbDate = client_date ? client_date : new Date().toISOString();
  ...
  const length = questionStr.length;
  ...
  const cost = amount != null ? Number(amount) : (rule.rows.length > 0 ? (rule.rows[0].coins as number) : 4);
  const label = rule.rows.length > 0 ? (rule.rows[0].label as string) : "Short";
  const spendSource = source || "voice_chat";
  const spendDesc = description || (length > 0 ? `Voice chat - ${label} (${length} chars)` : `Spent ${cost} coins`);
  ...
  await db.execute({
    sql: `INSERT INTO coin_transactions (user_id, amount, type, source, description, idempotency_key, created_at) VALUES (?, ?, 'SPEND', ?, ?, ?, ?)`,
    args: [user_id, -cost, spendSource, spendDesc, idempotency_key || null, dbDate]
  });
...
// ─── YOGA STAGES ──────────────────────────────────────────────
const DEFAULT_YOGA_LEVELS = [
  { level: 1, name: "Karma Yoga", min_coins: 0, max_coins: 999, multiplier: 1, description: "Foundational Action" },
  { level: 2, name: "Bhakti Yoga", min_coins: 1000, max_coins: 2999, multiplier: 2, description: "Path of Devotion" },
  { level: 3, name: "Jnana Yoga", min_coins: 3000, max_coins: 5999, multiplier: 2, description: "Path of Knowledge" },
  { level: 4, name: "Dhyana Yoga", min_coins: 6000, max_coins: 8999, multiplier: 3, description: "Path of Meditation" },
  { level: 5, name: "Raja Yoga", min_coins: 9000, max_coins: 9999, multiplier: 3, description: "Ultimate Mastery" },
];

const DEFAULT_SUB_STAGES = [
  { id: 1, level: 1, sub_level: 1, sub_name: "Karma Novice", min_coins: 0, max_coins: 249 },
  { id: 2, level: 1, sub_level: 2, sub_name: "Karma Practitioner", min_coins: 250, max_coins: 499 },
  { id: 3, level: 1, sub_level: 3, sub_name: "Karma Adept", min_coins: 500, max_coins: 749 },
  { id: 4, level: 1, sub_level: 4, sub_name: "Karma Master", min_coins: 750, max_coins: 999 },
  { id: 5, level: 2, sub_level: 1, sub_name: "Bhakti Novice", min_coins: 1000, max_coins: 1499 },
  { id: 6, level: 2, sub_level: 2, sub_name: "Bhakti Devotee", min_coins: 1500, max_coins: 1999 },
  { id: 7, level: 2, sub_level: 3, sub_name: "Bhakti Sadhaka", min_coins: 2000, max_coins: 2499 },
  { id: 8, level: 2, sub_level: 4, sub_name: "Bhakti Master", min_coins: 2500, max_coins: 2999 },
  { id: 9, level: 3, sub_level: 1, sub_name: "Jnana Seeker", min_coins: 3000, max_coins: 3749 },
  { id: 10, level: 3, sub_level: 2, sub_name: "Jnana Scholar", min_coins: 3750, max_coins: 4499 },
  { id: 11, level: 3, sub_level: 3, sub_name: "Jnana Philosopher", min_coins: 4500, max_coins: 5249 },
  { id: 12, level: 3, sub_level: 4, sub_name: "Jnana Sage", min_coins: 5250, max_coins: 5999 },
  { id: 13, level: 4, sub_level: 1, sub_name: "Dhyana Practitioner", min_coins: 6000, max_coins: 6749 },
  { id: 14, level: 4, sub_level: 2, sub_name: "Dhyana Meditator", min_coins: 6750, max_coins: 7499 },
  { id: 15, level: 4, sub_level: 3, sub_name: "Dhyana Contemplative", min_coins: 7500, max_coins: 8249 },
  { id: 16, level: 4, sub_level: 4, sub_name: "Dhyana Master", min_coins: 8250, max_coins: 8999 },
  { id: 17, level: 5, sub_level: 1, sub_name: "Raja Aspirant", min_coins: 9000, max_coins: 9332 },
  { id: 18, level: 5, sub_level: 2, sub_name: "Raja Master", min_coins: 9333, max_coins: 9665 },
  { id: 19, level: 5, sub_level: 3, sub_name: "Kaivalya Liberated", min_coins: 9666, max_coins: 9999 }
];
...
app.get("/yoga/stages", async (c) => {
  ...
  try {
    ...
    levelRows = levels.rows || [];
  } catch (_) {}

  try {
    const subStages = await db.execute("SELECT * FROM yoga_sub_stages ORDER BY level ASC, sub_level ASC");
    subRows = subStages.rows || [];
  } catch (_) {}

  if (!levelRows.length) levelRows = DEFAULT_YOGA_LEVELS;
  if (!subRows.length) subRows = DEFAULT_SUB_STAGES;

  return c.json({ levels: levelRows, sub_stages: subRows });
});
...
app.post("/activity/log", requireAuth, async (c) => {
  const user_id = c.get("userId" as any) as string;
  const { activity_type, source, description, metadata, client_date } = await c.req.json().catch(() => ({}));

  const actType = String(activity_type || source || "verse_read").trim();
  const desc = String(description || actType.replace(/_/g, " ")).trim();
  const dbDate = client_date ? client_date : new Date().toISOString();
  const todayIso = new Date().toISOString().split("T")[0];

  // 1. Record activity entry into coin_transactions
  await db.execute({
    sql: `INSERT INTO coin_transactions (user_id, amount, type, source, description, created_at)
          VALUES (?, 0, 'ACTIVITY', ?, ?, ?)`,
    args: [user_id, actType, desc, dbDate],
  }).catch((e) => console.error("Error logging activity transaction:", e?.message));

  // 2. Bump current_streak immediately (same rules as calculateStreak in /users/stats/sync)
  //    so the streak reflects today's activity right away instead of waiting for the
  //    next stats/sync call.
  const prev = await db.execute({
    sql: `SELECT current_streak, longest_streak, last_activity_date FROM user_stats WHERE user_id = ?`,
    args: [user_id],
  }).catch(() => ({ rows: [] as any[] }));

  let newStreak: number | null = null;
  let newLongest: number | null = null;
  if (prev.rows.length) {
    const row = prev.rows[0] as any;
    const prevDate = String(row.last_activity_date || "").substring(0, 10);
    const prevStreak = Math.max(0, (row.current_streak as number) || 0);
    const prevLongest = Math.max(0, (row.longest_streak as number) || 0);

    if (!prevDate) {
      newStreak = 1; // no known last-activity date — can't verify continuity, start fresh
    } else if (prevDate === todayIso) {
      newStreak = prevStreak; // already counted today
    } else {
      const sTime = new Date(prevDate + "T00:00:00Z").getTime();
      const tTime = new Date(todayIso + "T00:00:00Z").getTime();
      const dayDiff = isNaN(sTime) || isNaN(tTime) ? 0 : Math.floor((tTime - sTime) / (1000 * 60 * 60 * 24));
      if (dayDiff === 1 || dayDiff === 2) {
        newStreak = prevStreak <= 0 ? 1 : prevStreak + 1; // consecutive day (+2 TZ grace)
      } else if (dayDiff > 2) {
        newStreak = 1; // gap — reset, today starts a new streak
      } else {
        newStreak = prevStreak; // dayDiff <= 0, clock skew — leave alone
      }
    }
    newLongest = Math.max(prevLongest, newStreak);
  }

  // 3. Update user_stats: last_activity_date, verses_read if verse read, and the streak fields
  if (actType.includes("verse") || actType.includes("sloka")) {
    await db.execute({
      sql: `UPDATE user_stats
            SET verses_read = verses_read + 1,
                last_activity_date = ?,
                current_streak = COALESCE(?, current_streak),
                longest_streak = COALESCE(?, longest_streak),
                updated_at = datetime('now')
            WHERE user_id = ?`,
      args: [todayIso, newStreak, newLongest, user_id],
    }).catch(() => {});
  } else {
    await db.execute({
      sql: `UPDATE user_stats
  ...
  return c.json({ success: true, user_id, activity_type: actType, current_streak: newStreak });
});
...
app.get("/activity/history", requireAuth, async (c) => {
  ...
  const result = await db.execute({
    sql: `SELECT DATE(created_at) as date,
            COUNT(CASE WHEN source IN ('checkin_day', 'streak_checkin', 'checkin') THEN 1 END) as checkins,
            COUNT(CASE WHEN source IN ('quiz_completion', 'battle_quiz', 'quiz_attempt') THEN 1 END) as quizzes,
            COUNT(CASE WHEN source IN ('share_sloka', 'share') THEN 1 END) as shares,
            COUNT(CASE WHEN source IN ('meditation', 'meditation_session') THEN 1 END) as meditations,
            COUNT(CASE WHEN source IN ('voice_chat', 'voice_usage') THEN 1 END) as voice_chats,
            COUNT(CASE WHEN source IN ('verse_read', 'verse_reading', 'sloka_read') THEN 1 END) as verses_read,
...
app.post("/feedback", async (c) => {
  ...
  if (token) {
    ...
    if (session.rows.length) {
      user_id = session.rows[0].user_id as string;
    }
  }
  const { user_id: bodyUid, type = "feedback", subject = "", message, client_timestamp } = await c.req.json();
  if (!user_id) {
    user_id = bodyUid || "guest";
  }
...
// Deno Deploy sets DENO_DEPLOYMENT_ID; VPS uses PORT/HOST (default 0.0.0.0:8000)
const isDenoDeploy = !!Deno.env.get("DENO_DEPLOYMENT_ID");
if (isDenoDeploy) {
  console.log("Starting on Deno Deploy (PostgreSQL backend)");
