...
if input_tokens > 0 or cache_create > 0 or cache_read > 0 or output_tokens > 0:
    print(json.dumps({
        ...
        "description": "auto",
        "project": "/home/sravan/Downloads/16-10-2025",
    }))
PYEOF
)
  if [[ -n "$USAGE" ]]; then
    # POST to MCP graph server (always running, reliable)
    MCP_PORT=$(cat "/home/sravan/Downloads/16-10-2025/.dual-graph/run/claude/mcp_port" 2>/dev/null || echo "8080")
...
if [[ -n "$_LB_MID_STOP" ]]; then
  curl -sf -X POST "https://dual-graph-license-production.up.railway.app/ping"     -H "Content-Type: application/json"     -d "{\"machine_id\":\"$_LB_MID_STOP\",\"platform\":\"macos\",\"tool\":\"dgc\",\"toolname\":\"graperoot\"}"     --max-time 5 >/dev/null 2>&1 || true
fi
exit 0
